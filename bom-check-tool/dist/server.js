...
const SHEETS_CONFIG = {
  '一': {
    ...
    forbiddenFields: ['工程物料版本说明', '材料', '尺寸', '标准'],
    allowedValues: {
      '导入类型': ['I', 'D', 'U'],
      '物料类型': ['1', '2', '3'],
  ...
  '二': {
    ...
    forbiddenFields: ['原材料', '标准', '大小', '制造商', '备注 1', '备注 2', '备注 3', '客户物料开票名称'],
    allowedValues: {
      '导入类型': ['I', 'D', 'U'],
  ...
  '五': {
    ...
    requiredFields: ['导入类型', '产品行业', '行业细分', '产品理论重量', '产品应用', '项目名称'],
    forbiddenFields: ['物料开票名称', 'HS 编码', '密度', '料宽', '用金量', '机台效率', '模具原因', '麦途物料号'],
    allowedValues: {
      '导入类型': ['I', 'D', 'U']
    }
  }
...
function validateCrossSheetsSPM(sheetFour, sheetFive) {
  ...
  const fourPlatingSpeedIndex = sheetFour.headers.map(h => normalizeHeader(h)).findIndex(h => h === normalizeHeader('电镀产速 (m/min)'));
  ...
  const fivePlatingSpeedIndex = sheetFive.headers.map(h => normalizeHeader(h)).findIndex(h => h === normalizeHeader('电镀产速（m/min）'));
  ...
  console.log('[SPM 校验] 表五完整表头:', sheetFive.headers);
  console.log('[SPM 校验] 表五表头数量:', sheetFive.headers.length);
  
  console.log('[SPM 校验] 表四索引', { fourMfgIndex, fourProcessIndex, fourCycleIndex, fourPlatingSpeedIndex });
  console.log('[SPM 校验] 表四表头:', sheetFour.headers);
  console.log('[SPM 校验] 表五索引', { fiveMaterialIndex, fiveSPMIndex, fivePlatingSpeedIndex, fiveMoldCavityIndex, fiveInjectionCycleIndex });
  console.log('[SPM 校验] 表五前 15 行数据:', sheetFive.data.slice(0, 15).map(row => row.join('|')));
  
  // 调试：打印表四前 15 行数据
  console.log('[SPM 校验] 表四前 15 行数据:', sheetFour.data.slice(0, 15).map(row => row.join('|')));
  
  if (fourMfgIndex === -1 || fiveMaterialIndex === -1 || fiveSPMIndex === -1) {
    console.log('[SPM 校验] 必要字段索引缺失');
    return errors;
  }
  ...
  sheetFour.data.forEach((row, idx) => {
    ...
    if (mfg && String(mfg).trim() !== '') {
      ...
      fourDataMap.set(mfgStr, {
        ...
        platingSpeed: fourPlatingSpeedIndex !== -1 ? (row[fourPlatingSpeedIndex] || '') : ''
      });
    }
  });
  
  console.log('[SPM 校验] 表四数据量:', fourDataMap.size);
  console.log('[SPM 校验] 表四注塑数据示例:', Array.from(fourDataMap.entries()).filter(([k, v]) => v.process.includes('注塑')).slice(0, 5));
  console.log('[SPM 校验] 表四冲压数据示例:', Array.from(fourDataMap.entries()).filter(([k, v]) => v.process.includes('冲压')).slice(0, 5));
  ...
  sheetFive.data.forEach((row, idx) => {
    ...
    const fourPlatingSpeed = fourData.platingSpeed || '';
    ...
    if (isStamping) {
      ...
      if (isNaN(spmNum)) {
        console.log('[SPM 校验] 跳过 - SPM 非数字', { rowNumber, materialStr, spm });
        return;
      }
      ...
      // 只打印前 20 行日志
      if (matchCount <= 20) {
        console.log('[SPM 校验] 数据行', { rowNumber, materialStr, process, cycleStr, fourPlatingSpeed, spmNum: spmRaw, isStamping, isMolding, isPlating });
      }
    ...
    if (isMolding) {
      ...
      console.log('[注塑校验]', { rowNumber, materialStr, fiveMoldCavity, fiveInjectionCycle });
    ...
    // 电镀工艺校验
    else if (isPlating && fourPlatingSpeed) {
      const platingSpeedNum = parseFloat(fourPlatingSpeed);
      if (platingSpeedNum && !isNaN(platingSpeedNum)) {
        const expectedCycle = (1 / (platingSpeedNum * 1000 / 20)) * 1.05;
        if (cycleNum && !isNaN(cycleNum) && Math.abs(cycleNum - expectedCycle) > 0.01) {
          errors.push({
            row: rowNumber,
            field: '生产周期 (分钟)',
            value: cycleNum.toString(),
            error: `工艺流程包含"电镀"，表四生产周期应等于 [1/（电镀产速 (${platingSpeedNum})*1000/20）]*1.05=${expectedCycle.toFixed(4)}，当前值：${cycleNum.toFixed(4)}`
          });
        }
      }
    }
  ...
  console.log('[SPM 校验] 匹配数据行数:', matchCount, '电镀工艺行数:', platingCount, '错误数:', errors.length);
...
function validateSheetFiveSpecial(data, headers, startRowNumber) {
  ...
  data.forEach((row, rowIndex) => {
    ...
    // 分类禁填校验
    if (categoryIndex !== -1 && (suffixType === 'M' || suffixType === 'P')) {
      const categoryVal = row[categoryIndex];
      if (categoryVal !== null && categoryVal !== undefined && String(categoryVal).trim() !== '') {
        errors.push({
          row: rowNumber,
          field: '分类',
          value: String(categoryVal).trim(),
          error: `物料倒数第二位为"${suffixType}"，分类禁止填写`
        });
      }
    }
  });
  ...
  try {
    ...
    if (sheetFour && sheetFive) {
      ...
      if (crossSPMErrors.length > 0) {
        sheetFive.errors.push(...crossSPMErrors);
      }
    }
