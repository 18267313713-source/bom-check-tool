...
gen_card_slide(4, TOTAL_SLIDES, "三、表二：物料通用数据和客户料号", 
    ...
    basic_req=[
        {"label": "必填字段:", "items": ["导入类型, 物料号, 物料名称 (禁双空格), 物料类型", "物料组, 单位集, 单位, 重量单位, 产品类型", "产品分类, 物料信号, 商品代码, 项目经理", "批次控制 (1 或 2), 保质期/呆滞期, 计划/订货仓库", "计划数据计划员, 订货数据计划员, 订货数据车间计划员", "工艺路线代码、物料代码系统(固定 CW)、客户代码、业务伙伴物料代码 (全须填写)"]}
    ],
    forbid_req=[
    ...
    extra_logic=[
        "【豁免规则】若「物料号」包含 WX，则「保质期」不校验必填。",
        "【内部一致】计划数据默认仓库 == 订货数据仓库。",
        "【内部一致】计划数据计划员 == 订货数据计划员。",
        "【枚举】单位集: A01。计划员 / 订货计划员: JA153, JA303... 等 7 人。"
    ]
)
...
formulas = [
    ...
    ("3. 电镀工艺周期公式",
     "校验前提：表四工艺流程包含“电镀”。\n校验规则：表四「生产周期」 ≈ [ 1 / ( 电镀产速 × 1000 / 步距 ) ] × 1.05。\n说明：基于电镀速度和步距计算生产周期，并考虑 5% 的损耗，允许 10% 误差。")
]
