...
def add_industrial_slide(slide_idx, total):
    ...
    rules = [
        ...
        ("2. 净数量特殊逻辑 (表三)", "• 一般物料：净数量必须 > 0。\n• **特例**：组件代码末尾为 **FL / FLD** 或包含连续 **RR** 时，净数量**必须为负数** (如退料)。"),
        ("3. 防重复工序 (表四)", "同一个「制造物料」下，不允许存在重复的工序编号，确保工艺路线唯一。"),
        ("4. 废品率与生产周期限制", "• 表三废品率：禁止输入负数。\n• 表四生产周期：必须 > 0。"),
        ("5. 表五数值范围与逻辑", "• SPM、理论重量：必须 > 0。\n• 回料百分比：必须在 0~100 之间。\n• 模穴数：必须是正整数。\n• **重量约束**：产品塑胶重量 不能超过 产品理论重量。"),
        ("6. 表二关联校验", "• 若填写了「业务伙伴物料代码」，则「客户代码」必须同时填写。"),
        ("7. 表三结构组合唯一性", "• BOM 结构中，「工程物料」与「组件」的配对该表中必须是唯一的，防止重复粘贴导致冗余。"),
        ("8. 外协/X类物料免填规则", "• 表二：物料名称含「外协」或物料号含 -X → 保质期/呆滞期免填。\n• 表四：工艺流程说明含「外协」→ 机器免填。\n• 表五：物料含 -X → 产品理论重量免填。"),
        ("9. 表四外协工艺与T4任务绑定", "• 工艺流程说明含「外协」 ⇔ 任务前两位必须为 T4。\n• 仅满足一方而另一方不满足时均报错。")
    ]
